...
Section closure.
  ...
  (** The closure of a set is the intersection of all closed sets
      containing that set. *)
  ...
  Local Ltac t0 :=
    repeat match goal with
             | _ => eassumption
             | _ => progress (repeat (split || intro))
             | _ => progress (repeat unfold closure, ArbitraryIntersection, Closed, In in *; trivial)
             ...
             | _ => progress (hnf; repeat rewrite complement_idempotent)
  ...
  Local Ltac t1 tac :=
    t1' tac;
    match goal with
      | _ => apply NNPP; solve [ t1'' tac ]
    end.

  Lemma closure_closed_same S : Closed S -> closure S = S.
    t1 ltac:(exists (Complement X S)).
  Qed.

  Lemma closure_respect_subset U V : U ⊆ V -> closure U ⊆ closure V.
