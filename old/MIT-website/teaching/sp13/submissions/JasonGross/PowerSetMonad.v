...
Require Import Utf8.
Require Export Monad SetCategory PowerSetFunctor.
Require Import Common Notations Morphism Topology.
...
Section PowerSetMonad.
  ...
  Local Ltac t :=
    repeat (power_set_t || destruct_head_hnf @ArbitraryUnion).

  (** The function which takes an element [x : X] to the singleton
      subset [{x} : Ensemble X] is the function [@id x = fun y => (x =
      y)]. *)
  Let η : NaturalTransformation (IdentityFunctor CategoryOfTypes) PowerSetFunctor.
    refine (Build_NaturalTransformation (IdentityFunctor CategoryOfTypes) PowerSetFunctor
                                        (fun T x y => x = y)
                                        _).
    compute.
    abstract t.
  ...
  Let µ : NaturalTransformation (PowerSetFunctor o PowerSetFunctor) PowerSetFunctor.
    refine (Build_NaturalTransformation (PowerSetFunctor o PowerSetFunctor) PowerSetFunctor
                                        (ArbitraryUnion)
                                        _).
    compute;
      abstract t.
  ...
  Definition PowerSetMonad : Monad CategoryOfTypes.
    exists PowerSetFunctor η µ;
    compute; abstract t.
  Defined.
End PowerSetMonad.
