  ...
  Section Inverse.
    ...
    Definition InverseNaturalIsomorphism (T : NaturalIsomorphism F G) : NaturalIsomorphism G F
      := {|
          ...
          NaturalIsomorphism_Isomorphism := (fun x => InverseOf _ (NaturalIsomorphism_Isomorphism T x))
        |}.
  End Inverse.
 ...
 *)
Arguments NaturalEquivalenceOf [objC C objD D F G] T.
Arguments FunctorsNaturallyEquivalent [objC C objD D] F G.
...
Arguments NaturalEquivalenceOfCategories [objC C objD D] F G.
...
Section IdentityNaturalEquivalence.
  Context `(C : @Category objC).
  Context `(D : @Category objD).
  ...
  Qed.
End IdentityNaturalEquivalence.

Arguments IdentityNaturalEquivalence [objC C objD D] F x.
...
Hint Resolve functors_naturally_equivalent_refl functors_naturally_equivalent_sym : category functor.
Hint Immediate functors_naturally_equivalent_refl functors_naturally_equivalent_sym functors_naturally_equivalent_trans : category functor.
...
Section FunctorNaturalEquivalenceLemmas.
  ...
  Lemma LeftIdentityFunctorNE (F : Functor D C) : FunctorsNaturallyEquivalent (ComposeFunctors (IdentityFunctor _) F) F.
    match goal with
      | [ |- FunctorsNaturallyEquivalent ?a ?b ] => assert (H : a = b); eauto with functor; try (rewrite H; eauto with functor)
    end.
  Qed.
  ...
  Lemma ComposeFunctorsAssociativityNE (F : Functor B C) (G : Functor C D) (H : Functor D E) :
    ...
    match goal with
      | [ |- FunctorsNaturallyEquivalent ?a ?b ] => cut (a = b); try let H' := fresh in solve [ intro H'; rewrite H'; trivial || eauto with functor ]
...
Section CategoryNaturalEquivalenceRelation.
  ...
  Local Ltac t :=
    simpl; intros; apply Functor_Eq;
    repeat match goal with
             | _ => intro
             | _ => reflexivity
             | _ => progress simpl in *
             | [ |- context[match ?f ?x with _ => _ end] ] =>
               generalize (f x); intro; subst_eq_refl
           end.

  Lemma categories_naturally_equivalent_refl `(C : @Category objC) : CategoriesNaturallyEquivalent C C.
    repeat (exists (IdentityFunctor C)); split;
    match goal with
      | [ |- FunctorsNaturallyEquivalent ?a ?b ] => cut (a = b); try let H' := fresh in solve [ intro H'; rewrite <- H'; auto with category; reflexivity || trivial ]
    end;
    t.
  Qed.

  Lemma categories_naturally_equivalent_sym `(C : @Category objC) `(D : @Category objD) :
  ...
  Ltac solve_4_associativity :=
    match goal with
      | [ |- ?Rel _ (ComposeFunctors (ComposeFunctors ?a ?b) (ComposeFunctors ?c ?d)) ] =>
        eapply (functors_naturally_equivalent_trans (G := ComposeFunctors a (ComposeFunctors (ComposeFunctors b c) d)));
  ...
  Lemma categories_naturally_equivalent_trans `(C : @Category objC) `(D : @Category objD) `(E : @Category objE) :
