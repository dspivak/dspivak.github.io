...
Require Import Graph Category Isomorphism.
...
(** * Exercise 5.1.4.4 *)
Module Exercise_5_1_4_4.
  (** ** Problem *)
  (** Let [C = (G, ≃)] be a schema. A leaf table is an object [c : Ob
      C] with no outgoing arrows. Write this condition mathematically
      in three different languages: that of graphs (using symbols [V], [A],
      [src], [tgt]), that of categories (using [Hom_C], etc.), and that of
  ...
  Section GraphLeafTable.
    ...
    Let V := Vertex' G.
    Let A := Arrow' G.
    Let src := Graph'Source G.
    Let tgt := Graph'Target G.

    Definition GraphLeafTable (v : V) := forall a : A, src a <> v.
  End GraphLeafTable.

  Section CategoryLeafTable.
    Let Ob {objC} (C : @Category objC) := Object C.
    Let Hom {objC} (C : @Category objC) := Morphism C.
    Context `(C : @Category objC).

    Definition CategoryLeafTable (c : Ob C) :=
      forall d : Ob C, Hom C c d ≅ (c = d).
  End CategoryLeafTable.
End Exercise_5_1_4_4.
(** ------------------------------------------------------------------------ *)
