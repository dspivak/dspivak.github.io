...
Section interior.
  ...
  (** The interior of a set contains all points which are in some open
      set contained in [S] *)
  ...
  Lemma interior_open_same S : Open S -> interior S = S.
    intro H.
  ...
  Lemma interior_respect_subset U V : U ⊆ V -> interior U ⊆ interior V.
    t.
    match goal with
      | [ H0 : ?Xj ⊆ ?U, H1 : Open ?Xj, H2 : ?x ∈ ?Xj
          |- ?x ∈ interior ?V ]
        => (exists (Xj ∩ interior V))
    end;
      t;
      match goal with
        | _ =>
