...
function slideShow2(speed) {
  ...
  // append an 'li' item to the 'ul' list for displaying the caption
  $('ul.slideshow2').append('<li id="slideshow2-caption" class="caption"><div class="slideshow2-caption-container"><p></p></div></li>');
...
function gallery2() {
  ...
  if(current.queue('fx').length == 0) {
    ...
    // get the next image, if it reached the end of the slideshow, rotate it back to the first image
    var next = ((current.next().length) ? ((current.next().attr('id') == 'slideshow2-caption')? $('ul.slideshow2 li:first') :current.next()) : $('ul.slideshow2 li:first'));
