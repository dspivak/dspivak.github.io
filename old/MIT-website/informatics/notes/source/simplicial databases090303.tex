...
A {\em simplicial database (\tn{or simply} database) of type $\pi$} is a triple $(X,\mcR,\tau)$ where $X\in\Sch^\pi$ is a schema of type $\pi$, $\mcR\in\Shv(X)$ is a sheaf of sets on $\Sub(X)$, and $\tau\taking\mcR\to\mcU^\pi$ is a morphism of sheaves.  We refer to $X$ as {\em the schema}, $\mcR$ as {\em the keys}, and $\tau$ as {\em the data} of the database $(X,\mcR,\tau)$.
